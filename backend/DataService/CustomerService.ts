...
export async function getAllCustomersService(_req: Request, res: Response) {
  // add auth function here so data isn't sent freely ?

  const allCustomers = await formDB.getAllCustomers();
...
export async function findCustomerService(req: Request, res: Response) {
  ...
  const foundCustomer = formDB.findCustomer(customer);
